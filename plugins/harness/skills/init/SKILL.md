...
**機械的にできることは全部スクリプトが行う。** 散文手順をモデルに解釈させると実行のたびに
揺れる(手順の読み飛ばし・chmod 忘れ・settings.json の壊し方が毎回違う)。
...
- `scripts/survey.sh` — **読み取り専用**。`!` 記法でこのスキル読み込み時に自動実行され、
  ...
  調査を決定論化するのが目的(散文で「言語を確認せよ」と書くと読み飛ばしが起きる)。
...
- **コード配置**: rules の `paths:` に渡す glob。⚠️ **言語に合っていないと一度もマッチせず
  silent に無効化される**(swift-mcp-app で 2026-07-22 に実際に起きた)。調査結果の
  拡張子分布とトップ階層ディレクトリから決める。TypeScript なら `src/**,test/**,migrations/**`、
  Swift なら `Sources/**,Tests/**`。
