#!/usr/bin/env bash
# harness-template v0.17.0 — ベストプラクティス遵守を機械的に検査する(読み取り専用)。
#
# 設計意図(2026-08-05):
...
#   「常に exit 0」という契約は、SKILL.md の `!` 記法がこのスクリプトを**無条件**で
#   自動実行するために存在する(非0で終わるとスキル読み込み自体が壊れる)。ところが
#   出力を `head` 等で読み手側が打ち切ると、書き込み中の check.sh 自身が SIGPIPE を
#   受け取り、trap していなければ既定動作(プロセス終了)が働いて **141**(128+13)を
#   返してしまう —— 契約に反する非0の実例。
#   実測(このリポジトリで): `bash check.sh | head -3; echo "rc=${PIPESTATUS[0]}"` → rc=141。
#   直し方は `trap '' PIPE` で SIGPIPE を既定動作(終了)から無視に変える。
#   ⚠️ 副作用の検討: 無視した後、閉じたパイプへの書き込みは(シグナルで死ぬ代わりに)
#   `echo`/`printf` が EPIPE で失敗を返すだけになる。このスクリプトは**読み取り専用**
#   (ファイルへは1バイトも書かない。git ls-files 等の読み取りコマンドの結果を
#   標準出力へ流すだけ)なので、書き込み失敗が起きても実害があるのは「出力の続きが
#   見えない」ことだけで、状態が壊れる経路は無い。
#   ボツ案(Why not): `2>/dev/null` 等でエラー表示だけ抑える案 → SIGPIPE 自体は
#   シグナルであってエラー出力ではないので抑止できない(的外れ)。`set +o pipefail` を
#   外す案 → pipefail は check.sh **内部**のパイプ(`git ls-files | ...` 等)の終了コード
#   判定に使っており、今回の問題(外側の消費者が閉じる)とは無関係なので触らない。
#   実測の副作用: `trap '' PIPE` 適用後に `check.sh | head -3` すると rc=0 にはなるが、
#   閉じたパイプへの書き込みのたびに `printf: write error: Broken pipe` が**標準エラー**へ
#   複数回出る(EPIPE を無視せず bash 自身が都度メッセージ化するため)。これは exit code
...
#    SIGPIPE を無視した結果として書き込みが黙って失敗する経路があっても実害は
#    「出力が途中で見えなくなる」だけで、ファイルやリポジトリの状態には影響しない。
trap '' PIPE
...
finish() {
  ...
  # ⚠️ 版数は先頭の `harness-template v…` 行と**2箇所ある。手で揃えること**
  #    (2026-08-10 に実際にズレた —— 先頭だけ上げてこちらが旧版のまま出た)。
  echo "=== 検査完了: ${findings} 件(check.sh v0.17.0) ==="
  exit 0
}
