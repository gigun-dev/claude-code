...
#
# 設計意図(2026-08-05):
#   harness:audit は「立ち返るための手順書」で、実行するたびに人/モデルの解釈が入る。
#   こちらは**その中で機械判定できる項目だけ**を切り出した検査。スキルやハーネスを
#   作った直後に「本当に守れているか」を確定的に答えるのが目的。
#
#   判定基準の出典はベストプラクティス記事(harness:audit の references に要旨あり)。
#   ただし**閾値や規約は変わりうる**ので、audit 側で一次情報をライブ確認したときに
#   ここの数字も見直すこと。
#
...
if [ "$findings" -eq 0 ]; then
  ...
  echo "指摘 ${findings} 件。判定基準の出典と背景は /harness:audit を参照(閾値は一次情報で再確認できる)。"
fi
exit 0
