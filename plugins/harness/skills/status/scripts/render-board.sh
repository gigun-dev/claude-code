#!/usr/bin/env bash
# harness-template v0.5.0 (配布元: gigun-dev/claude-code plugins/harness)
...
#     詳細部の目次で、**どちらもボードを開いた人が最初に見るべきもの**だった。
...
#     タイル行は「未完了/完了」と「依存辺」だけになった(冒頭サイズは (3) のバーへ吸収)。
#
...
#
# (2) 冒頭サイズのバーを「トークン」1本だけにした(「文字」のバーを消した)。
...
#        nd-tasks.sh は同じ比較を**冒頭注入型のフックがあるリポジトリでだけ**行っている。
#        pointer 型(このリポジトリ自身がそう)では冒頭が SessionStart の stdout を通らず、
...
#        → **その結果、このリポジトリでは冒頭が何字でもバッジは出ない。**pointer 型だから。
#           バッジの実物は合成 ND(.claude/hooks に冒頭注入型のフックを置いたもの)で確認した。
...
ボードの読み方:
  ...
  冒頭サイズのバー = ND の冒頭。**トークン1本だけ**(v0.4.0。毎セッションの実費はトークンで、
             文字数は同じことの近似でしかない)。縦線が予算。目盛りは全ボード共通なので、
             ボード同士を長さで比較できる。
  ⚠ バッジ  = 冒頭が実際に切り詰められているときだけ出る(nd-tasks.sh の head-truncated を
             そのまま表示する)。SessionStart の stdout は 10,000字超で**無言で**切り詰め
             られる(#70460)—— 予算超過ではなく機能の破損なので、文字数はこの1点のために
             だけ残してある。⚠️ **pointer 型のフック(冒頭を stdout に通さない配線)では
  ...
  「現在地」と詳細部の目次は各ボードの下に**常時表示**(v0.2.0 で <details> を全廃した ——
...
# =============================================================================
# 冒頭の切り詰め(10,000字・#70460)—— **判定は自分でやらない**
# =============================================================================
...
#   pointer 型(このリポジトリ自身がそう)では、冒頭は SessionStart の stdout を通らない
...
def _cross_check_trunc():
    ...
    こちら側の再計算 = 「冒頭注入型 かつ head_chars > 10,000字」。nd-tasks の判定条件を
...
#              しかも「整理して降ろすほど誤検知が増える」という**運用に逆行する壊れ方**だった。
...
# =============================================================================
# ND から「現在地」と詳細部の目次を切り出す(本文は上の read_lines が既に読んである)
# =============================================================================
...
def section_toc(lines):
    """session-head-end マーカー以降の ## / ### を詳細部の目次として返す。

    リンクは張らない —— 目次が指すのは**同じ ND ファイルの節**であって、この HTML の中には
    その本文が無い(冒頭だけが予算制で、詳細部は on-demand に Read / grep するもの)。
...
def size_bars(meta):
    """冒頭サイズの表示。**常時出るのはトークンのバー1本だけ**(v0.4.0)。
...
for b in boards:
    ...
    board_html.append(
        ...
        '<div class="sec"><div class="lab">詳細部の目次 <small>(session-head-end 以降。本文は ND を Read / grep)</small></div>'
...
# --- タイル -------------------------------------------------------------------
# ⚠️ v0.2.0 で「次(<コンポーネント>)」と「冒頭サイズ」のタイルを外した。前者は ■ が各ボードの
...
meta_line = (
    ...
    '<span>バーの縦線 = 冒頭のトークン予算 %s tok(毎セッションの実費)</span>'
    % "{:,}".format(WARN_TOKENS)
)
...
if trunc_msg:
    meta_line += ('<span>⚠ = 冒頭が %s字を超え、SessionStart の stdout が無言で'
