...
def followup_prompt(report):
    ...
    「直前の本文を土台にする」「元の文章へ戻さない」を明示する理由(実測 2026-09-17):
    初版は「次の点だけを元の文章どおりに直し」と書いていた。これを、呼び出し元の
    指示が事実を落とす向き(数値と URL を省いた紹介文にせよ)だった条件で 5 回
    回すと、5 回とも書き直しが巻き戻り、うち 2 回は元の文章と 1 バイト違わない
    ものが返った。事実照合は通るのに何も直っていない状態になる。
    この 3 行へ書き換えて同じ条件で 5 回回したところ、元の文章と一致したものは
    0 回で、5 回とも書き直しを保ったまま指摘した値だけが戻った。
    """
    lines = [
        "直前に返してもらった本文で、元の文章にあった事実が変わっている箇所がある。",
        "直前に返してもらった本文を土台にして、次に挙げる値だけを元の文章の値へ直すこと。",
        "それ以外の文は直前の本文のまま残すこと。元の文章へ戻さないこと。",
        "直した本文の全体を、同じマーカー形式でもう一度返すこと。",
        "",
    ]
