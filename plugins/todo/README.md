...
採番(`new`。作るのは題と日付の 2 行)と、受理済み ADR に許された唯一の編集
(`supersede`)。どちらも 2026-09-10 に別のリポジトリで手作業になり、たまたま
正しかった。
...
adr supersede 古い 新しい   # 古い方の先頭に `Superseded by <slug>` を 1 行足す
```
...
- **`new` は番号を最大値 + 1 で採り、欠番は埋めない**(番号は採ったら改名しない)。
  slug は題を小文字にして `a-z0-9` 以外を `-` に畳んだもの。題が非 ASCII を含めば
  作らずに拒み、slug を求める —— 実際の題は日本語で、ASCII の断片だけを拾うと
  改名できない slug が残る。同じ番号や `NNNN-<slug>.md` でない名前が 1 つでも
  あれば書かない(`check` と同じ判定を使う。採番の当てにできないため)。
- **`supersede` は古い方に 1 行足すだけ**で、整形も書き換えもしない。新しい方が
  実在して題を持つこと・古い方がまだその行を持たず先頭行が題であることを確かめ、
  一つでも欠ければ何も書かずに終わる(半分だけ書かない)。
- 対話しない。データは stdout、診断は stderr。終了コード: **0** 成功 /
  **1** 引数・状態の誤り(`new` / `supersede` の拒否を含む)/ **2** `check` が
  指摘を出した。
...
出どころ・`check` の各指摘・ディレクトリの探索順・`ADR_DIR` の上書きに加え、
`new` の採番と slug と**拒否**(非 ASCII の題・番号の重複・読めないファイル名)、
`supersede` が足す 1 行と**拒否**(指す先が無い・既に持っている・先頭行が題でない)、
拒んだときに何も書いていないことを見る。日付だけ `ADR_TODAY` で固定する。
