...
- **本文の目標は短い文で 1〜3 文**(文脈・決定・理由。mattpocock/skills の
  元の形に戻した —— 段落数を縛った版は長い 1 文に押し込む結果になった)。
  `adr check` は文の数を縛らず、段落 4 つ以上(最大 3 段落という後備え)・
  見出し・箇条書き・番号付きリスト・表・コードフェンス・引用だけを指摘する。
  `adr stats` はファイルごとの文字数・段落数・文の数・最長文の文字数を
  多い順に報告する(合否は判定しない)。`adr check` の後に走らせ、突出した
  ものを split の候補として扱う。
...
adr stats                   # ファイルごとの文字数・段落数・文数・最長文を多い/長い順に報告する(ゲートではない)
...
- `check` が見るのは、ファイル名 / 番号の重複 / 題 / `Date:` / `Implementation:`
  ...
  `stats` は判定しない —— ファイルごとの文字数・段落数・文数・最長文の文字数を
  多い順に報告するだけ(常に 0 で終わる)。本文の目標は文脈・決定・理由を
  短い文で 1〜3 文だが、これは `check` が数えない(数えて縛ると、縛った単位の
  中に長い 1 文を詰め込む結果になる。段落を 1 つに縛った最初の版がそれで
  壊れた)。
...
文数・最長文の文字数・合計。句点ごとに文を数えること)・ディレクトリの
探索順・`ADR_DIR` の上書きに加え、`new` の採番と slug と
