...
- **本文は短い段落を最大 3 つ**(文脈・決定・理由。目安 120〜200 語)。
  2026-09-25 の実測(9 リポジトリ 249 件の ADR)で、最も軽い部類でも中央値
  約 160 語だった —— それ以前このプラグインが採っていた「1〜3 文」は
  その下位 10 パーセンタイルより軽く、costs や代替案が抜け落ちていた。
  `adr check` は語数を縛らず、段落 4 つ以上(最大 3 段落という後備え)・
  見出し・箇条書き・番号付きリスト・表・コードフェンス・引用に加え、
  **`Accepting:` が無い**(costs 抜きの Free Lunch Coupon)・**`Rejected:`
  に理由が無い**ことを指摘する。`adr stats` はファイルごとの文字数・語数・
  段落数・文の数・最長文の文字数を多い順に報告する(合否は判定しない)。
  `adr check` の後に走らせ、突出したものを split の候補として扱う。
- **`Accepting: <代償>` は必須(最低 1 行)。** `Rejected: <案> — <理由>` は
  任意・複数可(理由は必須)。`Revisit: <見直す条件>` は任意。
...
adr stats                   # ファイルごとの文字数・語数・段落数・文数・最長文を報告する(ゲートではない)
...
- `check` が見るのは、ファイル名 / 番号の重複 / 題 / `Date:` / `Implementation:`
  / `Status:` の語彙 / `Superseded by` の残骸 / `Accepting:` が最低 1 行あるか /
  `Rejected:` に理由(` — `)があるか / 構造(見出し・リスト・表・コードフェンス・
  引用・本文が 4 段落以上) / 内容(`[実測`・`[検査:`・`id:` 参照・`Date:` 行
  以外の日付)。**改名も書き換えもしない** —— 指摘だけを出す。`stats` は判定
  しない —— ファイルごとの文字数・語数・段落数・文数・最長文の文字数を多い順に
  報告するだけ(常に 0 で終わる)。本文の目安は文脈・決定・理由を短い段落
  最大 3 つ、120〜200 語(実測: 9 リポジトリ 249 件の ADR で最も軽い部類の
  中央値が約 160 語)だが、これは `check` が数えない(数えて縛ると、縛った
  単位に長文を詰め込む結果になる。1 段落・1〜3 文に縛った版が実際にそれで
  壊れた)。
...
の残骸・`Accepting:` が最低 1 行あるか・`Rejected:` に理由があるか(Accepting/
Rejected/Revisit は段落に数えないこと含む)・構造の各違反(本文 1〜3 段落は
通り 4 段落目から指摘)・内容の各違反とコードスパンの除外)・`stats` が
常に 0 で返ること(文字数の多い順・語数・段落数・文数・最長文の文字数・合計。
句点ごとに文を数えること)・ディレクトリの探索順・`ADR_DIR` の上書きに加え、
`new` の採番と slug と
