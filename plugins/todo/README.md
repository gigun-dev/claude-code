...
- **本文は 1 段落だけ**(文脈 → 決定 → 理由)。見出し・箇条書き・番号付き
  リスト・表・コードフェンス・引用・複数段落は `adr check` が指摘する ——
  文字数の上限は無く、軽さは形で強制する。
...
- `check` が見るのは、ファイル名 / 番号の重複 / 題 / `Date:` / `Implementation:`
  ...
  コードフェンス・引用・複数段落) / 内容(`[実測`・`[検査:`・`id:` 参照・
...
の残骸・構造の各違反・内容の各違反とコードスパンの除外)・ディレクトリの
探索順・`ADR_DIR` の上書きに加え、`new` の採番と slug と**拒否**(非 ASCII の
題・番号の重複・読めないファイル名)、`Replaces:` が名指しする削除済みの
番号を採番が再利用しないこと、`replace` が足す `Replaces:` トレーラ(複数回
の冪等な追記を含む)と古いファイルの削除・`supersede` が alias として同じ
挙動をすること・**拒否**(指す先が無い・自分自身・先頭行が題でない)と
