...
- **本文の目標は短い文で 1〜3 文**(文脈・決定・理由。mattpocock/skills が
  元にした「題 + 1〜3 文」という形に戻した —— 2026-09-25 に一度「1〜3 段落」
  へ広げたが、依然として軽くなかった。段落単位で縛っても文単位で縛っても、
  縛った単位の中に長い 1 文を詰め込まれるだけなので、`adr check` は文の数を
  数えない。収まらない・長くなるなら、複数の決定が同居しているか、実装の
  詳細(コードのそばのコメントかコミットメッセージ行き)の合図として扱う。
  段落は**最大 3 つまで**という構造上の後備えだけを `adr check` が見る
  (次節)。
...
- コードフェンス(` ``` `)
- 引用(`>`)
- 本文の段落が 4 つ以上(**目標の 1〜3 文からすれば十分緩い後備え**であって、
  4 段落未満なら軽いという意味ではない)

見出しや箇条書きが要る決定は、実際には複数の決定か、決定というより手順書
(それは docs や README の役目)。

`adr stats` は check とは別に、ファイルごとの文字数(`wc -m` 相当)・段落数・
文の数・最長の文の文字数を、文字数の多い順に報告する(合否は判定しない。
常に 0 で終わる)。文の数を check が縛らない分、`adr check` の後に
`adr stats` を走らせ、文が多い/長いものを split の候補として扱う。書いた
直後に限らず、軽さが安定するまでは定期的に見る。

## 文単位の関門(構造だけでは軽くならない)

3 段落までという構造の制約は、`adr check` が機械的に見られる範囲でしかない。
3 段落に収まっていても、その中に「覆すのが安いサブ決定」がいくつも束ねられて
いれば、それは軽い ADR ではない(check は通るが実質は重い)。この線引きは
構造では検査できないので、書く/レビューする側が**文ごとに**許可テストを
掛ける: その文は、この ADR が記録する**1 つの覆しにくい決定**について書かれて
いるか。
