...
**書く前に、1 文で理由を言えるか試す。** 言えなければ、複数の決定が混ざっているので分けるか、経緯ならコミットメッセージか measurements へ。本文は文脈・決定・理由を短い文で 1〜3 文 —— 収まらない/長くなるなら、複数の決定が同居しているか実装の詳細でコードコメント行き。**同じ関門は文ごとにも掛ける**: 覆すのが安いサブ決定を書いた文は実装コードのコメントかコミットメッセージへ移し、全文がそれで消えるなら ADR ごと消して理由をコードコメントへ。決定の理由そのものは削らない —— 削ると、それを支える前提ごと消える。
...
ファイルは `adr new "<題>" [<slug>]` で作る —— 採番・ファイル名・日付・`Implementation: pending` はこれが書く(題が非 ASCII を含むときは slug を渡す)。出力されたパスに本文(1〜3 文)を書き足し、`adr check` を通す。書いたら `adr stats` も走らせ、上位(文字数・文数・最長文が突出したもの)を split の候補として扱う —— 軽さが安定するまでは折に触れて見る。
