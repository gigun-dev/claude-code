...
| `dep:0001` | 依存。カンマ区切りで複数可。**指す先が done.txt に入るまで `ready` に出ない**（宙吊りの id を指す行は永久に出ない。`check` が名指しする） |
| `see:docs/x.md` | 根拠の置き場。**リポジトリ相対パス** —— URL は value にコロンが 2 つ目に入るので形式違反 |
| `id:0001` | `add` が採番する連番（4 桁ゼロ詰め。`done.txt` も数えた max+1）。**タスクを指す唯一の手段**（行番号は編集とマージでずれる）。指すときのゼロ詰めは任意（`todo do 12` = `id:0012`）で、数値の完全一致のみ。手で足した行には `todo id` が配る |
...
- **todo.txt は本線で更新する。worktree の implementer は報告し、`do` は検証した者が打つ。**
