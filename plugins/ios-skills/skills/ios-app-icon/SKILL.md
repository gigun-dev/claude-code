...
**ただし autofit が正解とは限らない。** これは「安全域を最大限使う」動作なので、
**余白そのものを構図に使っている案とは相性が悪い**。実例: 大きな面が左上に寄り
小さな面が右下へ流れる構図で autofit を掛けると 1.08 倍に広がって中央へ寄り、
大きな面が縁に迫って 80pt で重なりが読めなくなった。上と左に余白があるからこそ
視線が流れる構図だったので、`"autofit": false` にして手で置いた座標を採用した。
案が固まったら一度 false にして見比べること。
...
| ファイル | 内容 |
| --- | --- |
| `references/native-look.md` | **純正の質感の出し方**。実測した6要素、貼れる SVG レシピ、飴玉/硬い輪郭などの失敗、迷走しない案出しの進め方、生成元をコミットする規約 |
