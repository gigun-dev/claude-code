...
### 🔴 監査のチェックリスト(A/B で「見落とし」が出た項目)

**色の値を見るだけでは足りない。変数が本当に代入されているかまで見る。**

| 確認 | なぜ | 実例(実測) |
|---|---|---|
| **ホスト注入変数が `setProperty` されているか** | `var(--x, fallback)` は参照があっても**値が届いていなければ常にフォールバック**。CSS を読むだけでは気づけない | `setProperty("--color` が全ファイルで **0 件** = ホストのテーマが一度も適用されていなかった |
| **`var(--x)` の `--x` が定義されているか** | 名前の綴り違いは**エラーにならず静かにフォールバックする** | `--hairline` は定義 0・参照 2。似た名前の `--border-hair` が 15 件あった(綴り違い) |
| **同じトークンを「文字色」と「塗り」の両方に使っていないか** | 暗地向けに明るく振ったトークンを塗りに使うと、**その上の固定 `#fff` がダークだけ失格**になる | `#fff` on `--accent`: ライト 4.55 → **ダーク 2.71** |
| ライトも測ったか | 「ダークが読みにくい」は体感で、**実際はライトの方が悪いことがある** | `--text-3` はライト 2.07 / ダーク 2.51 |
