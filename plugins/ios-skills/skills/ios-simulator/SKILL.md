...
> 🔧 **訂正(2026-08-02・追試)**: 当初ここに「**一次情報だけが効く**」と書いたが、**言い過ぎだった。**
> **「何を見るべきか」の観点のカタログも転移する。** 追試で、監査チェックリスト5項目のすべてが
> 所見を生み、うち2件は**項目が無ければ発想自体が出なかった**と実行側が申告している。
> ただし条件つき —— **答えではなく観点を書くこと。**
> 答え(具体的な数値・変数名)を書くと、その eval は次から**記憶を測るだけ**になる(実際に壊した)。
