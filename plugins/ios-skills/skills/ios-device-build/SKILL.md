...
compatibility: >-
  ...
  devicectl を使う。Simulator は対象外(ios-simulator を使う)。
---

# iOS実機ビルド

対象を推測せず、プロジェクト・scheme・UDIDを明示して実行する。同じMacに他人の端末と他人の
ビルド成果物が混在している前提で動く。端末が特定できない、または`--dry-run`のplanが依頼と
...
4. stdoutの`status`、終了コード、実機上の起動結果を確認する。launch成功はプロセスが起動したことしか
   意味しないので、終了コードだけでE2E成功と判定しない。

スクリプトはstdoutへJSON、stderrへ診断、工程別の終了コードを返す。`--device`は環境変数
